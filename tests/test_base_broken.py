#!/usr/bin/env python3
"""
Base test infrastructure with proper isolation and cleanup.
...
# Import our comprehensive mocking infrastructure
try:
    from .mocks import (
        MockHookRunner, MockHookResponse, MockHookAction, MockSafetyHook,
        MockSubprocessRunner, MockSubprocessResult,
        MockFileSystem, MockFileSystemError,
        MockEnvironment, isolated_environment
    )
    MOCKS_AVAILABLE = True
except ImportError:
    # For when mocks module is not yet available, define minimal stubs
    MOCKS_AVAILABLE = False
    class MockHookRunner:
        def reset(self): pass
        def set_hook_response(self, *args): pass
        def run_hook(self, *args): return None
        def get_hook_calls(self, *args): return []
    class MockHookResponse:
        def __init__(self, action=None, **kwargs):
            self.action = action
    class MockHookAction:
        BLOCK = "block"
        ALLOW = "allow"
        ERROR = "error"
    class MockSafetyHook:
        def evaluate_tool_call(self, *args): return MockHookResponse()
    ...
    def isolated_environment(**kwargs):
...
class ComprehensiveMockTestCase(IsolatedTestCase):
    """
    Test case with comprehensive mocking infrastructure.
    
    Provides all mocking utilities: hooks, subprocess, filesystem, and environment.
    This replaces the old MockHookTestCase with enhanced capabilities.
    """
    
    def setUp(self):
        """Set up with comprehensive mocking capabilities."""
        super().setUp()
        ...
        self.mock_environment = MockEnvironment(isolated=False)
    ...
    def tearDown(self):
        """Clean up with mock system resets."""
    ...
    def _setup_patches(self):
        """Set up automatic patches for common operations.""" 
        # This can be extended later if automatic patching is needed
        # For now, we provide manual control over what gets mocked
        pass
    
    def enable_safety_evaluation(self):
        """Enable real safety logic evaluation in mock safety hook."""
        try:
            # Import safety logic if available
            scripts_path = Path(__file__).parent.parent / "scripts"
            if str(scripts_path) not in sys.path:
                sys.path.insert(0, str(scripts_path))
            import safety_logic
            self.mock_safety_hook.enable_auto_evaluation(safety_logic)
        except ImportError:
            # Safety logic not available, continue with manual mock responses
            pass
    
    # Convenience methods for hook mocking
    def set_hook_to_block(self, hook_name: str = 'pre_tool_use', reason: str = "Test blocked"):
        """Set hook to block with given reason."""
        response = MockHookResponse(MockHookAction.BLOCK, reason=reason, exit_code=1)
        self.mock_hooks.set_hook_response(hook_name, response)
    
    def set_hook_to_allow(self, hook_name: str = 'pre_tool_use'):
        """Set hook to allow."""
        response = MockHookResponse(MockHookAction.ALLOW)
        self.mock_hooks.set_hook_response(hook_name, response)
    
    def set_hook_to_error(self, hook_name: str = 'pre_tool_use', error_msg: str = "Test error"):
        """Set hook to error with given message."""
        response = MockHookResponse(MockHookAction.ERROR, reason=error_msg, exit_code=2, stderr=error_msg)
        self.mock_hooks.set_hook_response(hook_name, response)
    
    # Convenience methods for subprocess mocking
    def add_safe_command(self, command: str, output: str = "", return_code: int = 0):
        """Add a safe command with expected output."""
        result = MockSubprocessResult([command], return_code, stdout=output)
        self.mock_subprocess.add_command_response(command, result)
    
    def block_dangerous_commands(self):
        """Enable blocking of dangerous commands (default behavior)."""
        self.mock_subprocess.enable_dangerous_blocking()
    
    def allow_dangerous_commands(self):
        """Disable blocking of dangerous commands (use with extreme caution)."""
        self.mock_subprocess.disable_dangerous_blocking()
    
    # Convenience methods for filesystem mocking
    def create_test_file(self, path: str, content: str = ""):
        """Create a file in the mock filesystem."""
        self.mock_filesystem.write_file(path, content)
    ...
    def protect_file(self, path: str):
        """Make a file read-only in the mock filesystem."""
        self.mock_filesystem.make_read_only(path)
    
    def block_file_access(self, path: str):
        """Block access to a file path in the mock filesystem."""
        self.mock_filesystem.block_path(path)
    
    # Convenience methods for environment mocking
    def set_test_env_var(self, key: str, value: str):
        """Set an environment variable in the mock environment."""
        self.mock_environment.set(key, value)
    
    def protect_env_var(self, key: str):
        """Protect an environment variable from modification.""" 
        self.mock_environment.protect(key)
    
    def block_env_var(self, key: str):
        """Block access to an environment variable."""
        self.mock_environment.block(key)
    
    # Integration methods
    def test_tool_call_safety(self, tool_name: str, tool_input: Dict[str, Any]) -> MockHookResponse:
        """Test a tool call against safety logic and return the response."""
    ...
    def assert_hook_was_called(self, hook_name: str, expected_calls: int = 1):
        """Assert that a hook was called the expected number of times."""
        calls = self.mock_hooks.get_hook_calls(hook_name)
        self.assertEqual(len(calls), expected_calls, 
                        f"Expected {expected_calls} calls to {hook_name}, got {len(calls)}")
    
    def assert_command_was_executed(self, command_pattern: str):
        """Assert that a command matching the pattern was executed."""
        commands = self.mock_subprocess.get_executed_commands(command_pattern)
        self.assertTrue(len(commands) > 0, 
                       f"Expected command matching '{command_pattern}' to be executed")
    
    def assert_file_was_accessed(self, path: str, operation: str = None):
        """Assert that a file was accessed with the given operation."""
        operations = self.mock_filesystem.get_operations(operation)
        matching_ops = [op for op in operations if op['path'] == path]
        self.assertTrue(len(matching_ops) > 0,
                       f"Expected file '{path}' to be accessed with operation '{operation or 'any'}'")
    
    def assert_env_var_was_set(self, key: str, expected_value: str = None):
        """Assert that an environment variable was set."""
        changes = self.mock_environment.get_changes()
        set_changes = [c for c in changes if c.key == key and c.action in ['set', 'change']]
        self.assertTrue(len(set_changes) > 0, f"Expected environment variable '{key}' to be set")
        
        if expected_value is not None:
            latest_change = set_changes[-1]
            self.assertEqual(latest_change.new_value, expected_value,
                           f"Expected '{key}' to be set to '{expected_value}', got '{latest_change.new_value}'")


class MockHookTestCase(ComprehensiveMockTestCase):
    """
    Backward compatibility class for existing tests.
    
    DEPRECATED: Use ComprehensiveMockTestCase instead for new tests.
    This class maintains the old interface while providing new capabilities.
    """
    
    def setUp(self):
        """Set up with backward compatibility."""
        super().setUp()
